...
final class WorkoutManager: ObservableObject {
    ...
    func start() {
        if isPaused {
            ...
            // Konum servisini sıfırla ve kayda başla
            locationService?.resetDistance()
        }

        // Kayıt modunu aç
        locationService?.isRecording = true
    ...
    func pause() {
        ...
        // Kayıt modunu durdur (GPS çalışmaya devam eder ama mesafe eklemez)
        locationService?.isRecording = false
    ...
    func reset() {
        ...
        // Kayıt modunu ve mesafeyi sıfırla
        locationService?.isRecording = false
        locationService?.resetDistance()
    ...
    func addLap() {
        ...
        print(
            "[WorkoutManager] Lap \(lap.number) added: \(lap.formattedDistance) in \(lap.formattedTime)"
        )
    }
    ...
    private func setupLocationObserver() {
        ...
        // Hız değişimlerini takip et (Yumuşatılmış veri kullan)
